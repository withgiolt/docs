...
GlowVM is Giolt's runtime for Gleam's Erlang target — [AtomVM](https://www.atomvm.org) compiled to
...
Gleam on the Erlang target compiles to BEAM bytecode. GlowVM runs that bytecode on AtomVM built for
`wasm32-wasi` inside a JavaScript/WASI runtime on Giolt's infrastructure.

Your BEAM modules are packed into an AtomVM PackBeam (`.avm`) and executed by GlowVM. If it runs on
AtomVM it should run on GlowVM — GlowVM just adds a small compatibility layer where it fills gaps.
If it doesn't run on AtomVM, it likely won't run on GlowVM either.

For the JavaScript target on Giolt there is no emulation layer — it runs as normal JavaScript.
The limitations below apply only to the Erlang target via GlowVM.

## Limitations

GlowVM inherits most of AtomVM's limitations. The shims improve compatibility but do not yet provide
full OTP / BEAM parity.

### 1. Inherited AtomVM VM limits
...
### 2. OTP standard library coverage
...
[GitHub](https://github.com/withgiolt/glowvm).
